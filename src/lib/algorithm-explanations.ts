...
const EXPLANATIONS_SQ: Record<AlgorithmType, AlgorithmExplanation> = {
  ...
  'a-star': {
    ...
    name: 'Algoritmi A*',
    shortSummary:
      'A* eshte nje kerkim me drejtim qe kombinon koston reale me nje heuristike: `f(n) = g(n) + h(n)`.',
    bulletPoints: [
      'g(n) eshte kostoja e njohur nga burimi deri te nyja.',
      'h(n) eshte vleresimi i kostos se mbetur drejt qellimit.',
      'Me heuristike te mire, A* viziton me pak nyje se Dijkstra.',
      'Kur h(n) = 0, A* sillet si Dijkstra.',
      'Funksionon vetem me pesha jo-negative.',
    ],
    theory:
      'A* eshte nje algoritem i informuar i kerkimit qe perdor nje heuristike `h(n)` si parashikim te kostos se mbetur deri te qellimi. Ne cdo hap, ai llogarit `g(n)` si koston reale te deritanishme, `h(n)` si vleresim te mbetur, dhe `f(n) = g(n) + h(n)` si koston totale te pritur. Nese heuristika eshte e pranueshme dhe nuk e mbivlereson rrugen reale, A* mbetet optimal. Kjo eshte arsyeja pse nje heuristike e mire e ben kerkimin me te shpejte: ai fokusohet te nyjet qe duken me premtuese drejt qellimit. Kur `h(n) = 0`, A* kthehet praktikisht ne Dijkstra.',
    overview:
      'Mbaj nje radhe me prioritet me celes `f(n)`. Nxirr nyjen me `f` me te vogel, shfaq `g(n)`, `h(n)` dhe `f(n)`, pastaj relakso fqinjet. Kur qellimi nxirret nga radha, rinderto rrugen.',
    whenToUse: [
      'Navigim ne harta dhe GPS.',
      'Pathfinding ne lojra.',
      'Robotike dhe planifikim levizjeje.',
    ],
    dataStructures: [
      'Radhe me prioritet me celes f(n) = g(n) + h(n).',
      'Tabela gScore[v], hScore[v], dhe fScore[v].',
      'Tabela e prinderve per rindertim rruge.',
    ],
    timeComplexity: 'Varet nga heuristika; ne rastin me te keq sillet si Dijkstra.',
    spaceComplexity: 'O(V) ne rastin me te keq - mund te ruaje shume nyje ne open-set.',
    detailedSteps: [
      'Inicializo gScore[start]=0, h(start) dhe fScore[start]=gScore[start]+h(start).',
      'Shto start ne open-set me celes f(start).',
      'Pop nyjen u me f me te vogel.',
      'Shfaq g(n), h(n) dhe f(n) qe nxenesi te kuptoje pse u zgjodh ajo nyje.',
      'Nese u = qellimi, rinderto rrugen permes prev[].',
      'Per cdo fqinj v te u: nese g_provisore = gScore[u] + w < gScore[v], perditeso gScore[v], h(v), fScore[v] = gScore[v] + h(v), prev[v]=u.',
      'Perserit derisa qellimi te gjendet ose open-set te zbrazet.',
    ],
    exampleGraph: ASTAR_EXAMPLE,
    exampleWalkthrough: [
      'Burimi A, qellimi D. Mendoje kete si nje harte me koordinata: nyjet me te aferta me qellimin marrin h me te vogel.',
      'A fillon me g(A)=0 dhe me nje vleresim te dukshhem heuristik, keshtu qe f(A)=g+h.',
      'Kur A zgjerohet, B dhe C marrin vlera te ndryshme g, h dhe f. Nyja me f me te vogel vizitohet e para.',
      'Kur qellimi D del nga open-set, rruga rindertohet dhe mesazhi final tregon si u arrit.',
      'Nese modaliteti heuristik vendoset ne zero, h(n)=0 per te gjitha nyjet dhe A* sillet si Dijkstra.',
    ],
    commonMistakes: [
      'Te perdoret heuristike jo e pranueshme dhe te prishet optimaliteti.',
      'Te supozohet se A* eshte gjithmone me i shpejte, edhe kur heuristika eshte e dobet ose zero.',
      'Te konsiderohet nje nyje "perfunduar" pa e nxjerrre nga open-set.',
    ],
    limitations: [
      'Heuristika e dobet e ben A* te afrohet shume me Dijkstra.',
...
const EXPLANATIONS_EN: Record<AlgorithmType, AlgorithmExplanation> = {
  ...
  'a-star': {
    ...
    name: 'A* Algorithm',
    shortSummary:
      'A* is a goal-directed search that combines the real cost and a heuristic: f(n) = g(n) + h(n).',
    bulletPoints: [
      'g(n) is the known cost from the source to the current node.',
      'h(n) estimates the remaining cost to the goal.',
      'A better heuristic lets A* visit fewer nodes than Dijkstra.',
      'When h(n) = 0, A* behaves like Dijkstra.',
      'With an admissible heuristic, finds the optimal path.',
      'Requires non-negative weights.',
    ],
    theory:
      'A* is an informed search that uses a heuristic h(n) as an estimate of the remaining cost to the goal. For every node, it tracks g(n), the actual cost already paid, h(n), the estimated remaining cost, and f(n) = g(n) + h(n), the predicted total cost. If the heuristic is admissible, meaning it never overestimates the true cost, A* stays optimal. That is why heuristic quality matters: a better heuristic points the search toward promising nodes without changing the real path cost. When h(n) = 0, A* reduces to Dijkstra.',
    overview:
      'Maintain a priority queue keyed by f. Pop the smallest node, show g(n), h(n), and f(n), then relax neighbours. Reconstruct the path when the goal is popped. Good heuristics let the algorithm explore far fewer nodes than Dijkstra.',
    whenToUse: [
      'GPS and map navigation.',
      'Game pathfinding on grids.',
      'Robotics and motion planning.',
    ],
    dataStructures: [
      'Priority queue keyed by f(n).',
      'gScore[v], hScore[v], fScore[v].',
      'Parent table.',
    ],
    timeComplexity: 'Depends on the heuristic; same worst case as Dijkstra.',
    spaceComplexity: 'O(V) in the worst case.',
    detailedSteps: [
      'Init gScore[start]=0, h(start) from the heuristic, and fScore[start]=gScore[start]+h(start).',
      'Push start onto the open-set.',
      'Pop the node with smallest f.',
      'Show g(n), h(n), and f(n) so the chosen node is transparent to the learner.',
      'If it is the goal, reconstruct the path via prev[].',
      'For each neighbour v: if g_provisional = gScore[u] + w < gScore[v], update gScore[v], h(v), fScore[v]=gScore[v]+h(v), prev[v]=u.',
    ],
    exampleGraph: ASTAR_EXAMPLE,
    exampleWalkthrough: [
      'Source A, goal D. Imagine the graph is drawn on a plane. Nodes closer to the goal get smaller h(n) values.',
      'A starts with g(A)=0 and a visible heuristic value, so f(A)=g+h.',
      'When A expands, B and C receive their own g, h, and f values. The smaller f is chosen first.',
      'Once D is popped from the open-set, the path is reconstructed and the final message shows the selected route.',
      'If the heuristic mode is set to zero, h(n)=0 for every node and A* behaves like Dijkstra.',
    ],
    commonMistakes: [
      'Using a non-admissible heuristic and losing optimality.',
      'Assuming A* is always faster even when the heuristic is weak or zero.',
      'Treating a node as finalised before popping it.',
    ],
    limitations: [
      'A weak heuristic makes A* drift toward Dijkstra.',
